...
System.out.println("세션에서 가져온 사용자 정보: " + userVO);
if (userVO != null) {
    System.out.println("세션 사용자 ID: " + userVO.getId());
} else {
...
<body bgcolor="<%=bodyback_c%>">
	...
	<main>
		...
		<form method="post" name="writeForm" action="writeProc.jsp"
			...
			<input type="hidden" name="num" value="<%=num%>"> <input
				...
				name="writer" required> <label for="subject">제목</label>
			<td width="330">
				<%
				if (request.getParameter("num") == null) {
				%> <input type="text" id="subject" size="50" maxlength="50"
 ...
 %>
			...
			<label for="email">이메일</label> <input type="email" id="email"
				name="email" required> <label for="pass">비밀번호</label> <input
